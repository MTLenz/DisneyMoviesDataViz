...
function draw() {
  ...
  const x = 60;        // bar left
  const h = 180;        // ✅ fixed bar height (original height)
  ...
  for (let idx = 0; idx < rows.length; idx++) {
    ...
    if (filmstripImg) {
      ...
      // This guarantees the filmstrip fills the rect's height you had before.
    ...
    textSize(30);
    // text(`${movies[i]}`, x + w_resize + labelPad, y + h / 2);
